...
export default function CommentSection({ postId }: { postId: string }) {
  ...
  const handleAddComment = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // API 호출로 댓글 생성
      const response = await fetch(`http://localhost:8080/api/v1/curations/${postId}/comments`, {
        method: "POST",
        headers: {
  ...
  const handleEditSave = async (commentId: number) => {
    ...
    try {
      ...
      // API 호출로 댓글 수정
      const response = await fetch(`http://localhost:8080/api/v1/curations/${postId}/comments/${commentId}`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  const handleDeleteComment = async (commentId: number) => {
    ...
    try {
      ...
      if (result.code === "200-1") {
        // 댓글 목록에서 삭제된 댓글 제거
        setComments(comments.filter((comment) => comment.id !== commentId && comment.commentId !== commentId))
      } else {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        {comments.length > 0 ? (
          comments.map((comment, index) => (
            <div key={comment.commentId || comment.id || index} className="rounded-lg border p-4">
              <div className="flex justify-between">
                <div className="flex items-center space-x-2">
                  <Image
                    src={comment.authorImgUrl || "/placeholder.svg?height=36&width=36"}
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-500">
                      {comment.createdAt &&
                      comment.modifiedAt &&
                      Math.floor(new Date(comment.modifiedAt).getTime() / 1000) !==
                ...
                <div className="flex space-x-1">
                  {comment.id !== editingCommentId && comment.commentId !== editingCommentId ? (
                    <>
                      <button
                      ...
                      <button
                        onClick={() => handleDeleteComment(comment.commentId || comment.id || 0)}
                    ...
                    <>
                      <button
                        onClick={() => handleEditSave(comment.commentId || comment.id || 0)}
