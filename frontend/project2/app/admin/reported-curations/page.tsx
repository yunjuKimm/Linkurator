...
export default function AdminReportedCurationsPage() {
    ...
    const checkAdminAndFetchData = async () => {
      ...
      try {
        const response = await fetch(
          `http://localhost:8080/api/v1/admin/reported-curations?minReports=${minReports}`,
          {
            credentials: "include",
          }
        );
  ...
  const handleDeleteCuration = async (id: number) => {
    ...
    try {
      const response = await fetch(
        `http://localhost:8080/api/v1/admin/curations/${id}`,
        {
          method: "DELETE",
          credentials: "include",
        }
      ...
      setReportedCurations(
        reportedCurations.filter((curationId) => curationId !== id)
      );
